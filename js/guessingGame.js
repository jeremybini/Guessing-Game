...
function startGame() {
	...
	//click or enter button will submit answer
		...
		if (playersGuess.length<1) {
			...
			if (hintCount>0) {
				...
				if (hint===1) {
					...
					var closest = Math.abs(playersGuess[0]-winningNumber);
					for (var i = 0; i < playersGuess.length; i++) {
						var diff = Math.abs(playersGuess[i]-winningNumber);
						if (diff<closest) {
							closest = diff;
						}
					}
					var distance = Math.abs(winningNumber - closest);
					$('#hint').text('Your best guess was '+closest+' away.')
					...
					var hintAmount = 2*(5-playersGuess.length);
				...
				$('#hint').text("I think you've used enough hints...");
	...
	function winGame() {
		...
		$('#win').text('You Win!');
		$('#replay').show()
		$('#cover').show().addClass('coverWin');
		$('#cover').animate({opacity: 1}, 1500);
	};
